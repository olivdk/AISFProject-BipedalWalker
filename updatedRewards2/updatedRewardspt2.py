...
class CustomBipedalReward(gym.RewardWrapper):
    ...
    def reward(self, reward, observation):
        ...
        if (abs(observation[0]) > 0.3): reward -= 0.05 * abs(observation[0])
        if (abs(observation[0] > 0.5)): reward -= 0.1 * abs(observation[0])
        if (abs(observation[0] > 0.7)): reward -= 0.2 * abs(observation[0])
        ...
        if (observation[2] > 0.3): reward += 1.0 * observation[2]
        if (observation[2] > 0.5): reward += 1.5 * observation[2]
        if (observation[2] > 0.7): reward += 2.0 * observation[2]

        if observation[2] < 0: reward -= 1.0 * abs(observation[2])

        # Reward having straighter legs and moving knees faster
        knee_angle_indices = [6, 11]
        knee_speed_indices = [7, 12]

        for idx in knee_angle_indices:
            if abs(observation[idx]) < 0.7:
                reward += 0.20 * abs(observation[idx])

        for idx in knee_speed_indices:
            if abs(observation[idx]) > 0.5:
                reward += 0.05 * abs(observation[idx])
            if abs(observation[idx]) > 0.7:
                reward += 0.1 * abs(observation[idx])
            if abs(observation[idx]) > 1:
                reward += 0.15 * abs(observation[idx])
